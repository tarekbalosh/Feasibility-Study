 ...
 */
export async function classifyProject(name: string, description: string) {
  const prompt = `
  ...
  try {
    ...
    logger.error('Error in classifyProject', error);
    // fallback to generic
 ...
 */
export async function generateReport({ sector, data, financialResult, kpiResult }: any) {
  const prompt = `
  ...
  try {
    ...
    logger.error('Error in generateReport, falling back to mock generator', error);
    const html = generateMockReport(sector, data, financialResult, kpiResult);
    return { html, pdfUrl: null };
  }
}

function generateMockReport(sector: string, data: any, financialResult: any, kpiResult: any) {
  const name = data.name || 'مشروع جديد';
  const description = data.description || 'لا يوجد وصف للمشروع';
  
  // Format numbers nicely
  const formatNum = (num: number) => typeof num === 'number' ? Math.round(num).toLocaleString() : num;
